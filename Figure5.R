...
p1 <- ggplot(data)+
  ...
  geom_point(aes(y = co2_mean, x = dic,  fill = (season)), size = 2, shape = 21, stroke = 0.2)+
...
p2 <- ggplot(data)+
  geom_hline(yintercept = 0, linetype = "dashed")+
  geom_point(aes(y = co2_mean, x = ph,  fill = (season)), size = 2, shape = 21, stroke = 0.2)+
  scale_fill_manual(values =c("#009E73","#D55E00", "#CC79A7"))+
  ylab(((expression(paste("C", O[2] , " Flux (mmol ", m^-2, d^-1,")")))))+
...
ggsave(filename = 'figures/Figure5.png',width = 6,height = 4,units = 'in', dpi = 500)

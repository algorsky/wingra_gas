...
co2_winter_plot <- ggplot()+
  ...
  geom_point(data = filter(co2_winter, season == "ice"), aes(x = days_ice, y = CO2_mean, fill = as.factor(biomass)), 
    size = 2, shape = 21, stroke = 0.2)+
  geom_pointrange(data = filter(co2_winter, season == "ice"), aes(x = days_ice, y = CO2_mean, ymin = CO2_mean - CO2_sd, ymax = CO2_mean + CO2_sd, fill = as.factor(biomass)), shape = 21, stroke = 0.2)+
  #geom_point(data = filter(co2_winter, season == "open"), aes(x = days_ice, y = CO2_mean, fill = as.factor(biomass)), size = 2, shape = 21)+
  geom_pointrange(data = filter(co2_winter, season == "open"), aes(x = days_ice, y = CO2_mean, ymin = CO2_mean - CO2_sd, ymax = CO2_mean + CO2_sd,fill = as.factor(biomass)), shape = 21, stroke = 0.2)+
...
ggsave(filename = 'figures/Figure6.png', width = 6, height = 2.5, units = 'in', dpi = 500, bg = 'white')

...
# Lake locations
lakelocations = data.frame(lakeid = c('WI'),
                           Lake = c( "Lake Wingra"),
                           Lat = c(43.05258), 
                           Long =  -89.42499) 

# Joined pH, temp, and dic
df = read_csv("data/temp_dic_ph.csv")|>
  rename(temp = Temp_C)%>%
  rename(ph = pH)%>%
...
biomass_color<- read_csv("data/map/crosswalk_biomass.csv")

alk<- df2|>
  left_join(biomass_color, by = "site")
